...
namespace SmallBusinessManagementSystem.Repository
{
    public class SalesRepository
    {
        ...
        public List<CustomerModel> GetCustomrList()
        {
            ...
            _commandString = $"SELECT Code, CONCAT(Customer.Name,'(',Customer.Code,')') AS Name FROM {tableName}";
            ExecuteQuery();
        ...
        public double GetReorderLevel(SalesModel salesModel)
        {
            _commandString = $"SELECT ReorderLevel FROM Product WHERE Category = '{salesModel.Category}' AND Code = '{salesModel.Product}';";
            ExecuteQuery();
            double reorderLevel = 0;
            while (_sqlDataReader.Read())
            {
                reorderLevel = Convert.ToDouble(_sqlDataReader["ReorderLevel"].ToString());
            }
